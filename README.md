...
- **A fork-capable Waypoint build is required** (the concurrent-forking model with `checkpoint` / `fork` / `snapshot` / `exec <fork> --` commands). StateFork probes the binary at startup and refuses older `create`/`restore`-style builds with a clear error. The manager operates on one *current fork* (initially `main`): `snapshot()` seals it into an immutable checkpoint, `restore()` materializes a fresh fork from a checkpoint and switches to it, and the fork API (`fork(n)` / `exec_in_fork` / `snapshot_fork` / `destroy_fork` / `list_forks`) drives additional concurrent forks. See `scripts/waypoint_fork_demo.py` for an end-to-end tour.
